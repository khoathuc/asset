...
export default function CheckinConfirm({
  ...
  const confirm = async () => {
    try {
      const response = await axios.post(`/api/assets/${asset.id}/checkin`);
      if (response.data.success == false) {
        throw new Error(response.data.message);
      }

      toast.success("Checkin asset successfully");
    } catch (error) {
    ...
    <dialog id={dialogId} ref={ref} className="modal block overflow-auto pt-52">
      ...
      >
        <div className="border-b-2 ">
          ...
          <h3 className="text-lg font-bold">Confirm checkin</h3>
        </div>
          ...
          <div className="flex gap-4">
            ...
            <div className="flex flex-col gap-2">
              <h4 className="font-semibold">
                Do you confirm check in this asset
              </h4>
              <span className="text-sm font-extralight">
                The current assignee will not manage this asset
              </span>
            </div>
          </div>
